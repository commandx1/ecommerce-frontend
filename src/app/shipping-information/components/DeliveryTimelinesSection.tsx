...
export default function DeliveryTimelinesSection() {
  return (
    <section id="delivery-timelines" className="py-12 sm:py-16 lg:py-20 bg-white">
                ...
                <div key={s.step} className="flex items-start">
                  ...
                  >
                    {s.isLast ? (
                      ...
                      <span className="text-white font-bold text-sm sm:text-base">{s.step}</span>
                    )}
                  </div>
                  <div>
                    <h4 className="font-semibold text-gray-800 mb-1 text-sm sm:text-base">{s.title}</h4>
                    <p className="text-sm sm:text-base text-gray-600">{s.description}</p>
                    <p className="text-xs sm:text-sm text-gray-500 mt-1">{s.note}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="space-y-4">
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-semibold text-sm sm:text-base">Uber Direct Orders</span>
                    <span className="text-pale-lime font-bold text-base sm:text-lg">2:00 PM Local</span>
                  </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-semibold text-sm sm:text-base">Standard Shippo Orders</span>
                    <span className="text-pale-lime font-bold text-base sm:text-lg">4:00 PM EST</span>
                  </div>
                  <p className="text-blue-100 text-xs sm:text-sm">Next business day dispatch guaranteed</p>
                </div>
              </div>
            </div>
            ...
            {speedCards.map((card) => {
              ...
              return (
                <div key={card.title} className="text-center">
                  <div className={`${card.bg} rounded-xl p-4 sm:p-6 mb-3`}>
                    <Icon className="text-steel-blue w-8 h-8 sm:w-10 sm:h-10 mx-auto mb-2" />
                    <div className="text-2xl sm:text-3xl font-bold text-steel-blue">{card.value}</div>
                  </div>
