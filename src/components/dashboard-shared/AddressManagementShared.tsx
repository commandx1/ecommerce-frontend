    ...
    <div className="max-w-4xl space-y-8">
      ...
      {isEditing && currentAddress ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {addresses.length === 0 ? (
            <div className="col-span-full rounded-2xl border border-dashed border-border-strong bg-surface-elevated p-12 text-center">
            ...
            addresses.map((address) => (
              <div
                key={address.id}
                className={cn(
                  "relative rounded-2xl border bg-surface-elevated p-6 shadow-soft",
                  address.defaultAddress ? "border-brand/40 ring-1 ring-brand/35" : "border-border-soft",
                )}
              >
                {address.defaultAddress && (
                  <span className="absolute top-4 right-4 rounded-full bg-brand/10 px-2 py-1 text-[10px] font-bold text-brand uppercase">
                    Default
                  </span>
                )}
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-text-secondary">{address.fullName}</p>
                  </div>
                </div>
                <div className="mb-6 space-y-1 text-sm text-text-muted">
                  <p>{address.formattedAddress || address.addressLine}</p>
                  <p>{address.phoneNumber}</p>
                </div>
                <div className="flex items-center space-x-4 border-t border-border-soft pt-4">
                  <Button
                    type="button"
                    variant="link"
                    size="sm"
